...
{
    public class MessageQueuePublishing : IMessageQueuePublishing
    {
        ...
        public async Task<bool> SendMessage(string ExchangeName, string RoutingKey, byte[] message)
        {
            try
            {
                await Task.Run(() =>
